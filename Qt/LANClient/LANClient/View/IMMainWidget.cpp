...
*************************************************/
UserInfor IMMainWidget::getLocalMyInformation() const
{
    return m_myself;
}
...
*************************************************/
void IMMainWidget::changeMyInformation(const UserInfor &me)
{
    if (nullptr != m_mainCtrl)
...
//        IMFlockChatWidget *flockRoom = nullptr;
...
//        IMDiscussionChatWidget *discussionRoom = nullptr;
...
Description: 显示好友信息
*************************************************/
void IMMainWidget::showFriendInformation(const UserInfor &user)
{
    if (m_informationMap.contains(user.m_userID))
    {
        m_informationMap[user.m_userID]->setUserInfor(user);
...
Description: 显示自己信息
*************************************************/
void IMMainWidget::showMyInformation(const UserInfor &user)
{
    m_myself = user;
...
//            this, SLOT(createChatRoom(QListWidgetItem *)));
    connect(m_mainCtrl, SIGNAL(getFriendInformationSuccess(UserInfor)),
            this, SLOT(showFriendInformation(UserInfor)));

    connect(m_labelHead, SIGNAL(clicked()),
            this, SLOT(getMyInformation()));

    connect(m_mainCtrl, SIGNAL(getMyInformationSuccess(UserInfor)),
            this, SLOT(showMyInformation(UserInfor)));

    connect(m_mainCtrl, SIGNAL(changeMyInformationSuccess(UserInfor)),
            this, SLOT(setMyInformation(UserInfor)));
    ...
    connect(m_mainCtrl, SIGNAL(getFriendRequest(TalkMessage, UserInfor)),
            this, SLOT(receiveFriendRequest(TalkMessage, UserInfor)));
