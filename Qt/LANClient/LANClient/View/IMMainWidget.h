...
//    void insertDiscussionRoomMap(const QString &, IMDiscussionChatWidget*);
    ...
    // 改变自己信息（发送至服务器）
    void changeMyInformation(const UserInfor &);
    // 改变好友备注（发送至服务器）
...
//    bool addDiscussionButton(const DiscussionInformation & disInfo);
    ...
    // 显示好友信息
    void showFriendInformation(const UserInfor &);
    // 显示自己的信息
    void showMyInformation(const UserInfor &);
    // 设置个人信息
    void setMyInformation(const UserInfor &);
...
public:
    ...
    // 接收并且处理好友的请求
    void receiveFriendRequest(const TalkMessage &, const UserInfor &);
